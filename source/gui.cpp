...
// Clear Text Buffers.
void Gui::clearTextBufs(void) { C2D_TextBufClear(sizeBuf); }
...
void Gui::sprite(int key, int x, int y, float ScaleX, float ScaleY) {
  C2D_DrawImageAt(C2D_SpriteSheetGetImage(sprites, key), x, y, 0.5f, NULL,
                  ScaleX, ScaleY);
}
...
void drawBatteryTop(void) {
  ...
  if (R_SUCCEEDED(PTMU_GetBatteryChargeState(&batteryChargeState)) &&
      batteryChargeState) {
...
bool Gui::promptMsg(const std::string &promptMsg, const std::string &AOption,
                    const std::string &BOption) {
  Gui::clearTextBufs();
  C3D_FrameBegin(C3D_FRAME_SYNCDRAW);
  C2D_TargetClear(Top, BLACK);
  C2D_TargetClear(Bottom, BLACK);
  ...
  Gui::DrawString(10, 180, 0.6f, WHITE,
                  "<A>: " + AOption + "   <B>: " + BOption, 400);
  Gui::DrawBottom();
  C3D_FrameEnd(0);
...
// Display a Helperbox.
void Gui::HelperBox(const std::string &Msg) {
  Gui::clearTextBufs();
  C3D_FrameBegin(C3D_FRAME_SYNCDRAW);
  Gui::setDraw(Top);
...
void Gui::DrawStringCentered(float x, float y, float size, u32 color,
                             const std::string &Text, int maxWidth) {
  Gui::DrawString(
      (currentScreen ? 200 : 160) + x -
          (std::min(maxWidth, (int)Gui::GetStringWidth(size, Text)) / 2),
      y, size, color, Text, maxWidth);
}

// Draw String or Text.
void Gui::DrawString(float x, float y, float size, u32 color,
                     const std::string &Text, int maxWidth) {
  C2D_Text c2d_text;
  ...
  C2D_DrawText(
      ...
      std::min(size, size * (maxWidth / Gui::GetStringWidth(size, Text))), size,
      color);
}

// Get String or Text Width.
float Gui::GetStringWidth(float size, const std::string &Text) {
  float width = 0;
...
void Gui::GetStringSize(float size, float *width, float *height,
...
float Gui::GetStringHeight(float size, const std::string &Text) {
  float height = 0;
