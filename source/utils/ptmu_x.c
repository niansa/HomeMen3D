...
Result mcuInit(void) { return srvGetServiceHandle(&mcuhwcHandle, "mcu::HWC"); }

Result mcuExit(void) { return svcCloseHandle(mcuhwcHandle); }

static Handle ptmuxHandle;
static int ptmuxRefCount;

Result ptmuxInit(void) {
  if (AtomicPostIncrement(&ptmuxRefCount))
    return 0;
  Result res = srvGetServiceHandle(&ptmuxHandle, "ptm:u");
  if (R_FAILED(res))
    AtomicDecrement(&ptmuxRefCount);
  return res;
}

void ptmuxExit(void) {
  if (AtomicDecrement(&ptmuxRefCount))
    return;
...
Result PTMUX_GetAdapterState(u8 *out) {
  ...
  if (R_FAILED(ret = svcSendSyncRequest(ptmuxHandle)))
    return ret;
