...
void MainMenu::DrawMenu(void) const {
  ...
  drawBatteryTop();
  Gui::DrawString(
      397 - Gui::GetStringWidth(0.6f,
                                "Titles: " + std::to_string(selectedTitle + 1) +
                                    " / " + std::to_string(maxTitles)),
      237 - Gui::GetStringHeight(
                0.6f, "Titles: " + std::to_string(selectedTitle + 1) + " / " +
                          std::to_string(maxTitles)),
      0.6f, WHITE,
      "Titles: " + std::to_string(selectedTitle + 1) + " / " +
          std::to_string(maxTitles));
  ...
  for (int i = 0; i < ENTRIES_PER_SCREEN && i < maxTitles; i++) {
    if (screenPos + i == selection) {
      Gui::Draw_Rect(50 + (i * 60) - 1, 160 - 1, 50, 50,
                     C2D_Color32(0, 200, 0, 255));
    }
    C2D_DrawImageAt(GameManagement::installedTitles[screenPos + i]->icon(),
                    50 + (i * 60), 160, 0.5f); // Installed Title Icon.
  }

  Gui::DrawStringCentered(
      0, 75, 0.7f, BLACK,
      GameManagement::installedTitles[selectedTitle]->name());
  Gui::DrawStringCentered(
      0, 105, 0.7f, BLACK,
      GameManagement::installedTitles[selectedTitle]->Author());
...
void MainMenu::Logic(u32 hDown, u32 hHeld, touchPosition touch) {
  ...
  if (ScreenMode == 1) {
    ...
    if (hDown & KEY_RIGHT) {
      if (selectedTitle == (int)GameManagement::installedTitles.size() - 1 ||
          selectedTitle == maxTitles - 1) {
        if ((int)GameManagement::installedTitles.size() > maxTitles &&
            selectedTitle > maxTitles - 1) {
    ...
    if (hDown & KEY_LEFT) {
      if (selectedTitle == -1) {
        selectedTitle = (int)GameManagement::installedTitles.size() < maxTitles
                            ? GameManagement::installedTitles.size() - 1
                            : maxTitles - 1;
        selection = (int)GameManagement::installedTitles.size() < maxTitles
                        ? GameManagement::installedTitles.size() - 1
                        : maxTitles - 1;
        ...
        selectedTitle = (int)GameManagement::installedTitles.size() > maxTitles
                            ? maxTitles - 1
                            : (int)GameManagement::installedTitles.size() - 1;
        selection = (int)GameManagement::installedTitles.size() > maxTitles
                        ? maxTitles - 1
                        : (int)GameManagement::installedTitles.size() - 1;
      } else {
    ...
    if (hDown & KEY_A) {
      if (!GameManagement::installedTitles.empty()) {
        ...
        APT_PrepareToDoApplicationJump(
            0, GameManagement::installedTitles[selectedTitle]->ID(),
            MEDIATYPE_SD);
